...
const StatusBadge = ({ status }: { status: Status }) => {
    ...
    return (
        <div className={cn("flex items-center gap-1.5 text-14-regular w-max px-2 py-0.5 rounded-full", st)}>
            <Image src={img} alt="img" width={15} height={15} className="w-4 h-4" />
            <span>{status}</span>
        </div>
    );
};
